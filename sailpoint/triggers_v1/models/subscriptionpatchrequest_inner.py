...
class SubscriptionpatchrequestInner(BaseModel):
    ...
    def op_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['add', 'remove', 'replace', 'move', 'copy']):
            warnings.warn(f"must be one of enum values ('add', 'remove', 'replace', 'move', 'copy') unknown value: {value}")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "op": obj.get("op"),
            "path": obj.get("path"),
            "value": SubscriptionpatchrequestInnerValue.from_dict(obj["value"]) if obj.get("value") is not None else None
        })
        return _obj
