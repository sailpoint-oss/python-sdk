# coding: utf-8

"""
    Identity Security Cloud API - Machine Identity Lifecycle Actions

    Use these APIs to submit, list, retrieve, and cancel machine identity lifecycle actions (for example agent activate and deactivate). We encourage you to join the SailPoint Developer Community forum at https://developer.sailpoint.com/discuss to connect with other developers using our APIs.

    The version of the OpenAPI document: v1
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from sailpoint.machine_identities_lifecycle_actions.models.lifecycle_action_submit_comment import LifecycleActionSubmitComment

class TestLifecycleActionSubmitComment(unittest.TestCase):
    """LifecycleActionSubmitComment unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> LifecycleActionSubmitComment:
        """Test LifecycleActionSubmitComment
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `LifecycleActionSubmitComment`
        """
        model = LifecycleActionSubmitComment()
        if include_optional:
            return LifecycleActionSubmitComment(
                comment = 'Suspending agent until security review completes'
            )
        else:
            return LifecycleActionSubmitComment(
                comment = 'Suspending agent until security review completes',
        )
        """

    def testLifecycleActionSubmitComment(self):
        """Test LifecycleActionSubmitComment"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
