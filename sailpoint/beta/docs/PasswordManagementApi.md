...
> PasswordDigitToken generate_digit_token(password_digit_token_reset)
...
with beta.ApiClient(configuration) as api_client:
    ...
    api_instance = beta.PasswordManagementApi(api_client)
    password_digit_token_reset = {userId=Abby.Smith, length=8, durationMinutes=5} # PasswordDigitTokenReset | 

    try:
        # Generate a digit token
        api_response = api_instance.generate_digit_token(password_digit_token_reset)
