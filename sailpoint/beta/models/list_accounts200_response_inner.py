...
class ListAccounts200ResponseInner(BaseModel):
    ...
    if TYPE_CHECKING:
        ...
        actual_instance: Any
    any_of_schemas: List[str] = Field(LISTACCOUNTS200RESPONSEINNER_ANY_OF_SCHEMAS, const=True)
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        if not isinstance(v, SlimAccount):
            error_messages.append(f"Error! Input type `{type(v)}` is not `SlimAccount`")
        else:
            return v

        # validate data type: FullAccount
        if not isinstance(v, FullAccount):
            error_messages.append(f"Error! Input type `{type(v)}` is not `FullAccount`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in ListAccounts200ResponseInner with anyOf schemas: FullAccount, SlimAccount. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> ListAccounts200ResponseInner:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ListAccounts200ResponseInner with anyOf schemas: FullAccount, SlimAccount. Details: " + ", ".join(error_messages))
        else:
            return instance
