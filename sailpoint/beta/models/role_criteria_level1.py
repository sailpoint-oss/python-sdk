...
class RoleCriteriaLevel1(BaseModel):
    ...
    children: Optional[conlist(RoleCriteriaLevel2)] = Field(None, description="Array of child criteria. Required if the operation is AND or OR, otherwise it must be left null. A maximum of three levels of criteria are supported, including leaf nodes. Additionally, AND nodes can only be children or OR nodes and vice-versa.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> RoleCriteriaLevel1:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> RoleCriteriaLevel1:
        """Create an instance of RoleCriteriaLevel1 from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return RoleCriteriaLevel1.parse_obj(obj)

        _obj = RoleCriteriaLevel1.parse_obj({
            "operation": obj.get("operation"),
            "key": RoleCriteriaKey.from_dict(obj.get("key")) if obj.get("key") is not None else None,
            "string_value": obj.get("stringValue"),
            "children": [RoleCriteriaLevel2.from_dict(_item) for _item in obj.get("children")] if obj.get("children") is not None else None
        })
        return _obj
