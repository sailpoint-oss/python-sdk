...
class Configuration:
    ...
    def __init__(
        ...
        self._base_path = "https://sailpoint.api.identitynow.com/v3" if host is None else host
        """Default Base url
        """
        ...
        # if(self.get_config() == False):
        #     raise Exception("No configuration options set. Please set configuration through environment variables or a local json config file.")

    @classmethod
    def environment_config(self)-> Configuration:
        if(os.environ.get('SAIL_BASE_URL')):
            self.host = os.environ.get('SAIL_BASE_URL')
        if(os.environ.get('SAIL_CLIENT_ID')):
            self.client_id = os.environ.get('SAIL_CLIENT_ID')
        if(os.environ.get('SAIL_CLIENT_SECRET')):
            self.client_secret = os.environ.get('SAIL_CLIENT_SECRET')
        
        self.token_url = str(self.host) + '/oauth/token'
    ...
    @classmethod
    def get_config(self):
        envConfiguration = self.environment_config()

        if(envConfiguration.client_id):
           return True
