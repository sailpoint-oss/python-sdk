...
class IdentityReference(BaseModel):
    """
    The manager for the identity.  # noqa: E501
    """
    ...
    @classmethod
    def from_json(cls, json_str: str) -> IdentityReference:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> IdentityReference:
        """Create an instance of IdentityReference from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return IdentityReference.parse_obj(obj)

        _obj = IdentityReference.parse_obj({
            "type": obj.get("type"),
            "id": obj.get("id"),
            "name": obj.get("name")
        })
