...
slug: /tools/sdk/python/v1/methods/requestable-objects
tags: ['SDK', 'Software Development Kit', 'Requestable_Objects', 'Requestable_Objects']
---

# sailpoint.requestable_objects_v1.RequestableObjectsV1Api
...
from sailpoint.requestable_objects_v1.api_client import ApiClient
from sailpoint.requestable_objects_v1.models.requestableobject import Requestableobject
from sailpoint.requestable_objects_v1.models.requestableobjectrequeststatus import Requestableobjectrequeststatus
...
with ApiClient(configuration) as api_client:
    ...
    statuses = '''[sailpoint.requestable_objects_v1.Requestableobjectrequeststatus()]''' # List[Requestableobjectrequeststatus] | Filters the result to the specified status/statuses, where each status is one of `AVAILABLE`, `ASSIGNED`, or `PENDING`. Specifying this parameter without also specifying an `identity-id` parameter results in an error.  SailPoint may add additional statuses in the future without notice. (optional)
    ...
    try:
        ...
        results = RequestableObjectsV1Api(api_client).list_requestable_objects_v1()
        # Below is a request that includes all optional parameters
        # results = RequestableObjectsV1Api(api_client).list_requestable_objects_v1(identity_id, types, term, statuses, limit, offset, count, filters, sorters)
        print("The response of RequestableObjectsV1Api->list_requestable_objects_v1:\n")
        for item in results:
            print(item.model_dump_json(by_alias=True, indent=4))
    except Exception as e:
        print("Exception when calling RequestableObjectsV1Api->list_requestable_objects_v1: %s\n" % e)
```
